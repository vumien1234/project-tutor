...
import { fetchTutorList } from "../api";
...
const TutorListManage = () => {
  const dispatch = useDispatch();

  // Lấy dữ liệu gia sư từ Redux Store
  const danhSachGiaSu = useSelector((state) => state.admin.tutorList);
  const isGetTutorList = useSelector((state) => state.admin.isGetTutorList);

  // Trạng thái để lưu tên tìm kiếm và modal
  const [searchText, setSearchText] = useState("");
  const [selectedUser, setSelectedUser] = useState(null); // Lưu thông tin gia sư đã chọn
  const [isModalVisible, setIsModalVisible] = useState(false); // Trạng thái modal

  // Gọi API để lấy danh sách gia sư khi chưa có dữ liệu
  useEffect(() => {
    if (!isGetTutorList) {
      dispatch(fetchTutorList());
    }
  }, [dispatch, isGetTutorList]);
        ...
        {selectedUser && (
          <div>
            ...
            <Row gutter={16}>
              ...
              <Col span={16}>{selectedUser.subjects || "Chưa có thông tin"}</Col>
            </Row>
            <Row gutter={16}>
              <Col span={8}>
...
export default TutorListManage;
