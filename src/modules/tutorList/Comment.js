...
const Comment = () => {
  ...
  useEffect(() => {
    dispatch(fetchComment({ page: currentPage }));
  }, [currentPage, dispatch]);

  const handlePostComment = async () => {
    if (postComment.trim()) {
      const params = {
        username_voted: currentUser.username,
        comment: postComment
      };
        ...
        <div className="py-12 w-full">
          ...
          <div className="my-10 p-8 border border-gray-200 rounded-lg relative">
            <div className="flex items-center space-x-4">
              <img src={currentUser?.avatar} className="w-[60px] h-[60px] rounded-full" alt="avatar" />
              <textarea
              ...
              <div className="w-full">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-5">
                    <h6 className="text-gray-400">{convertTime(cmt.created_at)}</h6>
                  </div>
                </div>
                <h6>{cmt.comment}</h6>
              </div>
